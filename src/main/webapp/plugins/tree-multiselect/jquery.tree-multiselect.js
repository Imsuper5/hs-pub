(function($) {
	...
	$.fn.treeMultiselect = function(opts) {
		...
		this
				.each(function() {
					...
					checkPreselectedSelections($originalSelect,
							$selectionContainer, options);
					...
					updateSelectedAndOnChange($selectionContainer,
							$selectedContainer, $originalSelect, options);

					armRemoveSelectedOnClick($selectionContainer,
	...
	function generateSelections($originalSelect, $selectionContainer, options) {
		...
		$originalSelect.find("> option").each(
				function() {
					var $option = $(this);
					var path = $option.attr('data-section').split(
					...
					var option = new Option(optionValue, optionName,
	...
	function fillSelections($selectionContainer, data) {
		function createSection($sectionContainer, title, num) {
	...
	function addDescriptionHover($selectionContainer) {
		...
		var targets = $selectionContainer
				.find("div.item[data-description!=''][data-description]");
	...
	function checkPreselectedSelections($originalSelect, $selectionContainer) {
		...
		var $selectedOptionDivs = $selectionContainer.find("div.item")
				.filter(
						function() {
							var item = $(this);
		...
		$selectedOptionDivs.find("> input[type=checkbox]")
				.prop('checked', true);
	}

	function armTitleCheckboxes($selectionContainer) {
		var $titleCheckboxes = $selectionContainer
				.find("div.title > input[type=checkbox]");
	...
	function uncheckParentsOnUnselect($selectionContainer) {
		...
		$checkboxes.change(function() {
			...
			if ($checkbox.is(":checked"))
				return;
			var $sectionParents = $checkbox.parentsUntil($selectionContainer,
					"div.section");
			$sectionParents.find("> div.title > input[type=checkbox]").prop(
					'checked', false);
		});
	}
			...
			sections
					.each(function() {
						...
						var numSelected = $items.filter(function() {
							var item = $(this);
							return item.find("> input[type=checkbox]").prop(
									'checked');
						}).length;

						var $sectionCheckbox = $section
								.find("> div.title > input[type=checkbox]");
	...
	function addCollapsibility($selectionContainer, options) {
		...
		$("span.collapse-section").unbind().click(
				function(e) {
					e.stopPropagation();
					var $collapseSection = $(this);
					var indicator = $collapseSection.text();
					$collapseSection
							.text(indicator == hideIndicator ? expandIndicator
									: hideIndicator);
	...
	function createSelectAllButtons($selectionContainer, options) {
		...
		var $checkboxes = $selectionContainer.find("div.item").find(
				"> input[type=checkbox]");
	...
	function updateSelectedAndOnChange($selectionContainer, $selectedContainer,
		...
		function createSelectedDiv(selection) {
			...
			$(item).attr('data-index', selection.initialIndex).appendTo(
					$selectedContainer);
		}
		...
		function updateOriginalSelect() {
			...
			$originalSelect.html($originalSelect.find("option").sort(
					function(a, b) {
		...
		function elToSelectionObject($el) {
			...
			var sectionName = $.map(
					$el.parentsUntil($selectionContainer, "div.section").get()
							.reverse(), function(parentSection) {
		...
		var initialRun = true;
		function update() {
			var $selectedBoxes = $selectionContainer.find("div.item").has(
			...
			if (initialRun) {
				...
				options.onChange(selections, newlyAddedSelections,
						newlyRemovedSelections);
			}
	...
	function armRemoveSelectedOnClick($selectionContainer, $selectedContainer) {
		$selectedContainer.find("span.remove-selected").unbind().click(
				function() {
					var value = $(this).parent().attr('data-value');
					var $matchingSelection = $selectionContainer
							.find("div.item[data-value='" + value + "']");
					var $matchingCheckbox = $matchingSelection
